...
describe("Web setup", () => {
  it("writes the selected member channel and configures the workspace Web Chat target", async () => {
    ...
    expect(fake.prompter.note).toHaveBeenCalledWith(
      "Run `vercel dev --local` from the project root to start Web Chat and every agent.",
      "Next steps",
      { tone: "success" },
    );
  });
  ...
  it("explains how to run Next-owned Web Chat locally", async () => {
    const effects = deps();
    const fake = createFakePrompter();
    const ctx = createSetupContexts({
      appRoot: "/project",
    ...
    await applyWebSetup({ hosting: "next", packageManager: "pnpm" }, ctx.apply, effects);

    expect(fake.prompter.note).toHaveBeenCalledWith(
      "Run `pnpm dev:web` from the project root to start Web Chat and every agent.",
      "Next steps",
      { tone: "success" },
    );
  });
