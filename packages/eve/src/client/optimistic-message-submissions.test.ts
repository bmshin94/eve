...
function received(message: string, deliveryIds: readonly string[]): MessageStreamEvent {
  const event = stampTestEvents([
    createMessageReceivedEvent({ message, sequence: 0, turnId: `turn_${deliveryIds[0]}` }),
  ])[0]!;
  return { ...event, meta: { ...event.meta, deliveryIds } };
}
