...
successful results await delivery. With `taskDeliveryPolicy: "cohort"`, eve holds successful completion notifications
...
Channel sessions default to `taskDeliveryPolicy: "auto"`; schedules default to
`"cohort"` and can override the policy on their own definition. See
[Background task results](/docs/channels/overview#background-task-results).

With `"auto"`, a successful result can start a parent turn while other tasks are
