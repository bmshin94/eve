...
successful results await delivery. With `taskWakePolicy: "cohort"`, eve holds successful completion notifications
...
Channel sessions default to `"individual"`; sessions started by schedules default
to `"cohort"`. An explicit `taskWakePolicy` on the channel overrides either default. See
[Background task results](/docs/channels/overview#background-task-results) for a
channel configuration example.

With `"individual"`, a successful result can start a parent turn while other tasks
are still running. Results from the same cohort already queued when the parent
becomes available may share a turn; this setting does not guarantee one turn per
task. The reporting context includes the results delivered in that turn, without
including earlier results again. Tasks continue to run independently.

The setting applies to background work owned by sessions on that channel,
including background tools and built-in, declared, and remote subagents. Child
sessions use their own channel's policy for the work they start.
