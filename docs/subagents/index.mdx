...
cohort. This is the default `tasks.wakePolicy: "cohort"` behavior.

To deliver independent results without waiting for unfinished siblings, set
`tasks.wakePolicy` on the **parent** agent:
...
The setting applies to all background tasks owned by that agent, including
background tools and built-in, declared, and remote subagents. A declared child
configures its own wake policy for the background work it starts.
