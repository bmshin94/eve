...
`eve add channel/web` installs Web Chat under `apps/web/` for a standalone project or a selected workspace member. Run it from `agents/<name>/`, or select the member explicitly from the workspace root:

```bash
eve add channel/web --agent support
```

Setup asks whether Next.js should own the agent lifecycle or Vercel should run Web Chat and the agents as peer services. For peer services, run `vercel dev --local` from the project root to start the complete service graph. For Next-owned hosting, run the generated `dev:web` script from the project root, for example `pnpm dev:web`; `eve/next` starts the discovered agents. Do not run bare `next dev` from the project root because the Next.js application and its `tsconfig.json` are under `apps/web/`.

`eve dev` remains an agent-only command. In a workspace, run it with `--agent <name>` when you want to start one agent without Web Chat.
