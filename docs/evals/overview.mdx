...
Use `setup` to start shared resources or run migrations before eve starts the local agent server. Return environment overrides and an optional `teardown` function:
...
`startTestDatabase` is your own helper; it should return only after the database is ready and migrations have finished. `setup` can also return nothing when it only needs to perform initialization.

- Setup runs once per `eve eval` invocation, after discovery and config loading, before target startup. It does not run for `--list` or when no evals are selected. Read setup environment values inside `test(t)`, not at module scope: eval modules are imported during discovery.
...
- Teardown runs after the local server and its sandbox handles shut down, including after startup or eval failures. Overrides remain available during teardown and are restored afterward, even if teardown throws. Cleanup failures fail the command.
- `--skip-report` does not disable setup or teardown. With `--url`, both run locally; overrides cannot change the remote server's environment.

If setup throws before returning, it must clean up any resources it already created. The per-eval `timeoutMs` does not apply to setup or teardown.
