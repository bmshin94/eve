...
- Teardown runs after the local server and its sandbox handles shut down, including after startup or eval failures. Overrides remain active until the CLI process exits. Cleanup failures fail the command.
