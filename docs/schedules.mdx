...
interface ScheduleDefinition {
  cron: string;
  taskDeliveryPolicy?: "auto" | "cohort"; // defaults to "cohort"
...
You can write the same thing as a plain `.md` file: its frontmatter takes `cron` and an optional `taskDeliveryPolicy`, and the body is the prompt.
...
Schedules default to `taskDeliveryPolicy: "cohort"`: successful background task
results wait for overlapping tasks to settle before waking the parent. Set
`taskDeliveryPolicy: "auto"` on the schedule to let the parent inspect ready results
and report them when they are useful independently of unfinished work:
...
export default defineSchedule({
  ...
  markdown: "Prepare the daily reports. Share useful results as they become available.",
});
```

The option also works on handler schedules and in Markdown frontmatter. A
schedule's policy applies to the sessions it starts, including through Slack or
another channel. The schedule's default `"cohort"` takes precedence over a channel
configured as `"auto"`; an explicit schedule `"auto"` takes precedence over a
channel configured as `"cohort"`.
