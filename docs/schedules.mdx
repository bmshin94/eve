...
Schedule-started sessions default to `"cohort"`: successful background task results
wait for overlapping tasks to settle before waking the parent. This includes
handler schedules that send through Slack or another channel. An explicit
`taskWakePolicy` on that channel takes precedence; set it to `"individual"` to
report ready results while other tasks continue. See
[Background task results](./channels/overview#background-task-results).

A scheduled task-mode session stays alive while it owns pending background tasks,
even when `"individual"` lets it report a completed result first.
