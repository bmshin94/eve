...
Web Chat installs a shared Next.js application at `apps/web` for either a standalone agent or a selected member of an agent workspace. Setup asks whether Next.js should host the agent or agents, or whether Vercel should run the frontend and agents as peer services; peer services are recommended and used by non-interactive setup. Existing authored Next.js or Vercel configuration is preserved, and eve stops with manual composition guidance instead of overwriting it. The setup result prints the local development command for the selected hosting mode. See [Project Structure](./concepts/project-structure) for browser-app placement and local development, and [Next.js](./guides/frontend/nextjs) for manual integration.
