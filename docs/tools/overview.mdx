...
Channel sessions default to `"individual"`: ready results can wake the parent while
other tasks continue. Results already queued from the same cohort may share a turn,
and the reporting context includes only the results delivered in that turn.
Schedule-started sessions default to `"cohort"`: successful completions wait for
overlapping tasks to settle, then arrive together. An explicit `taskWakePolicy` in
the [channel configuration](/docs/channels/overview#background-task-results) overrides
either default. User questions, input requests, failures, cancellation, and explicit
`task.postMessage` calls do not wait for the cohort under either policy.
