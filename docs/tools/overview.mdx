...
Schedules default to `"cohort"`: successful completions wait for overlapping tasks
to settle, then arrive together. Set `taskDeliveryPolicy` on the
[channel](/docs/channels/overview#background-task-results) or
[schedule](/docs/schedules#background-task-results) that starts the session;
schedules use their own policy even when sending through another channel.
User questions, input requests, failures, cancellation, and explicit
