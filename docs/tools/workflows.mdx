...
async function chooseTarget(task: string, criteria: Record<"researcher" | "operator", string>) {
  "use step";
...
The parent model receives `route_task`, but not `researcher` or `operator`. When it calls `route_task`, the workflow reads each effective subagent description from `ctx.agents` and JEV returns the typed `"researcher" | "operator"` choice. The workflow passes that choice directly to `ctx.agent()`, and the subagent's result becomes the `route_task` tool result.
