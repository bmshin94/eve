...
Define the model-visible router under `agent/tools/`:

```ts title="agent/tools/route_task.ts"
import { autoRouter } from "eve/tools/auto-router";

export default autoRouter();
```

The parent model receives `route_task`, but not `researcher` or `operator`. Its input is `{ message: string, outputSchema?: object }`. With two or more available subagents, `autoRouter()` sends the message and every effective description to JEV, then passes the selected path-derived name and message to `ctx.agent()`. It invokes a sole available subagent without evaluation. An optional `outputSchema` is forwarded to the selected agent.

`autoRouter()` uses every declared local, remote, and dynamic subagent available to the workflow. It does not include the built-in root-copy `agent` target. Use an authored `defineWorkflowTool` when the router should consider only a subset or needs different routing instructions. Build custom JEV criteria directly from `ctx.agents`:
...
return ctx.agent(result.answers.route.choice, { message: task });
```

`ctx.agents` is a replay-stable metadata snapshot taken when the workflow starts. It includes agents hidden from the parent model with `tool: false` or `disableTool()`, but exposes no model definitions, credentials, or callbacks. Invocation still checks the agent's availability through `ctx.agent()`.
