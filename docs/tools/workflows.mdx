...
Use an authored workflow tool when the parent model should decide to delegate, but JEV should choose the specialist. Set `tool: false` on each specialist so the parent model sees only the routing tool. The specialists remain callable by their path-derived names through `ctx.agent()`:

```ts title="agent/subagents/researcher/agent.ts"
import { defineAgent } from "eve";

export default defineAgent({
  description: "Investigate, analyze, and explain a task.",
...
export default defineAgent({
  description: "Carry out operational changes for a task.",
  ...
  const result = await evaluate({
    ...
    questions: {
      route: {
        ...
        criteria: {
          research: "Investigation, analysis, and explanation",
          operations: "Execution and operational changes",
        },
      },
    },
  });
...
export default defineWorkflowTool({
  ...
  async execute({ task }, ctx) {
    ...
    const route = await chooseTarget(task);
    return ctx.agent(targets[route], { message: task });
  },
});
```

The parent model receives `route_task`, but not `researcher` or `operator`. When it calls `route_task`, JEV returns the typed `"research" | "operations"` choice, and the workflow maps that choice to the corresponding subagent. The subagent's result becomes the `route_task` tool result.
