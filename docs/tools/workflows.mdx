...
The parent model receives `route_task`, but not `researcher` or `operator`. When it calls `route_task`, JEV returns the typed `"researcher" | "operator"` choice. Because each choice key matches a path-derived subagent name, the workflow passes it directly to `ctx.agent()`. The subagent's result becomes the `route_task` tool result.
