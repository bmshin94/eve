...
Use an authored workflow tool when the parent model should decide to delegate, but JEV should choose the specialist. Define the routing descriptions once under `agent/lib/`; the object keys match the subagents' path-derived names:

```ts title="agent/lib/specialists.ts"
export const specialistDescriptions = {
  researcher: "Investigation, analysis, and explanation",
  operator: "Execution and operational changes",
} as const;
```

Set `tool: false` on each specialist so the parent model sees only the routing tool. The specialists remain callable through `ctx.agent()`:

```ts title="agent/subagents/researcher/agent.ts"
import { defineAgent } from "eve";
import { specialistDescriptions } from "../../lib/specialists";

export default defineAgent({
  description: specialistDescriptions.researcher,
...
export default defineAgent({
  description: specialistDescriptions.operator,
  model: "openai/gpt-5.6-sol",
  tool: false,
});
```

Define the model-visible router under `agent/tools/`. Put the evaluation in a step so workflow replay records its result instead of making the routing request again:

```ts title="agent/tools/route_task.ts"
import { evaluate } from "eve/experimental/evaluate";
import { defineWorkflowTool } from "eve/tools";
import { z } from "zod";
import { specialistDescriptions } from "../lib/specialists";

async function chooseTarget(task: string) {
  "use step";
  ...
  const result = await evaluate({
    ...
    questions: {
      route: {
        ...
        criteria: specialistDescriptions,
      },
    },
  });
...
The parent model receives `route_task`, but not `researcher` or `operator`. When it calls `route_task`, JEV returns the typed `"researcher" | "operator"` choice. Because each description key matches a path-derived subagent name, the workflow passes the choice directly to `ctx.agent()`. The subagent's result becomes the `route_task` tool result.

Keep shared metadata in `agent/lib/` instead of importing a subagent's `agent.ts` into the router. Importing `agent.ts` couples the workflow bundle to the child model definition, while the shared object contains only the names and descriptions needed for routing.
