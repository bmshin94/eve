...
Set `taskDeliveryPolicy` on the channel to choose how background results are reported:

- `"auto"` (default): ready results can invoke the parent while other tasks continue. The parent reports results that are useful independently and stays silent when a useful answer needs unfinished work.
- `"cohort"`: wait for overlapping tasks to settle, then deliver their successful results together.
...
`"auto"` controls user-visible reporting after the model wakes; it does not avoid
model calls for partial results. Results already queued from the same cohort may
share a turn. Each reporting turn receives the whole cohort, including pending
tasks and available outputs, so previously withheld results remain available for
a later combined report.

The option is available on `defineChannel`, `eveChannel`, and the platform channel
wrappers. It covers background tools and subagents owned by sessions started on
that channel. Child sessions use their own policy.

[Schedules](../schedules#background-task-results) choose their own
`taskDeliveryPolicy`, defaulting to `"cohort"`. That schedule policy takes
precedence over the destination channel, including when the schedule omits it.

Failures, cancellations, input requests, and explicit task messages remain
eligible for delivery under either policy. User messages follow `turnPolicy`.
