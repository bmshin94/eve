...
background-task results that arrive during another send. Runtime-authored task input stays in model
context instead of rendering as a participant message. A turn settling does not close the connection. The transport reconnects from its cursor after a disconnect or the
